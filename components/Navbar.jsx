...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      scrolled ? 'backdrop-blur-2xl bg-[#020810]/80 border-b border-white/6 shadow-xl shadow-black/50' : 'bg-transparent'
    }`}>
      <nav className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <ul className="hidden md:flex items-center gap-8">
          {links.map(({ to, label }) => {
            ...
            return (
              <li key={to}>
                <Link href={to} className={`relative text-sm font-medium transition-colors duration-200 ${active ? 'text-sky-400' : 'text-slate-300 hover:text-white'}`}>
                  {label}
                  {active && <motion.span layoutId="nav-indicator" className="absolute -bottom-1 left-0 right-0 h-0.5 rounded-full bg-gradient-to-r from-sky-400 to-blue-600" />}
                </Link>
              </li>
            )
          })}
          <li>
            <Link href="/contact" className="glow-btn px-5 py-2 rounded-xl text-sm font-semibold bg-gradient-to-r from-sky-500 to-blue-600 text-white">
              Get Quote
            </Link>
          </li>
        </ul>
        ...
        {open && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <ul className="flex flex-col px-6 py-4 gap-4">
              {links.map(({ to, label }) => (
                <li key={to}>
                  <Link href={to} className={`block text-base font-medium transition-colors ${pathname === to ? 'text-sky-400' : 'text-slate-200 hover:text-white'}`}>
                    {label}
                  </Link>
                </li>
              ))}
